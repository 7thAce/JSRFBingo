...
function launchGameReader(multiID) {
    ...
    nodecgServer.send("connect", "Connected");
            ...
            case 2:
                ...
                switch (jsonData.sub) {
                    ...
                    case 2:
                        ...
                        HandleTapeCollect(jsonData.dw1, this);
                        break;
                    case 3:
...
function HandlePlayerRegister(playerName, playerIndex, teamObj) {
    ...
    if (kevingoSocket && playerName != '') {
        kevingoSocket.send();
    } // I don't remember waht this is for but it might be necessary.

    nodecgServer.send("automarker_forward", JSON.stringify({type : "set_automark", data : {names : playerNames}}));
}
...
function HandleTapeCollect(tapeID, teamObj) {
    ...
    nodecgServer.send("tape_collected", {
        "teamID": teamObj.multiID,
        "tapeID": tapeID,
        // "player": teamObj.players[playerIndex];
    });
...
function HandleSoulUnlock(soulNum, teamObj) {
	...
	nodecgServer.send("soul_unlocked", {
		"teamID": teamObj.multiID,
		"soulID": soulNum
	});
	return;
...
function HandleCharUnlock(charID, playerIndex, teamObj) {
    nodecgServer.send("char_unlocked", {
        "teamID": teamObj.multiID,
        "charID": charID,
        "player": teamObj.players[playerIndex]
    });
